...
- **Conditional N-to-1 Semantic Consolidation (CRITICAL):** The Stage 1 ledger is a coverage inventory, not an artifact-generation list. You MUST consolidate multiple inventory items under a single primary semantic owner (like a single FS-MPF step or Section 8 table) ONLY when the behaviors belong to the same execution stage and consolidation does not obscure execution order, branching, intermediate outcomes, cardinality, termination, or query transitions. You MUST NOT consolidate when those sequences become obscured. Do not create 1-to-1 artifacts merely to prove coverage if they genuinely share a step.
...
**CONDITIONAL DATA-MAPPING TEST:** Structural translations such as wrapper creation, null-substitution, enum conversions, and passive field population must be primarily routed to `DS-REQ`, `DS-RESP`, or Section 4 flow. Do NOT classify these as `FS-BRL`. `FS-BRL` is strictly reserved for independently enforceable business decisions, precedence rules, and calculations.
...
*Conditional N-to-1 Consolidation Reconciliation:* Apply the Conditional N-to-1 Consolidation rule defined above. Consolidation is permitted only when the contributing behaviors belong to the same execution stage and does not obscure execution order, branching, intermediate outcomes, cardinality, termination, or query transitions. Otherwise, do not consolidate.
...
6. **Mechanical Conformance & Referential Integrity Gate (AUTHORITATIVE):** Before producing the final FSD, perform a private generation-time conformance check. Enforce exact HTML `<!-- TRACE -->` syntax (no Markdown approximations like `**TRACE:**`), literal column schemas for Section 8 and Appendices A/B, correct artifact placement, and literal completeness-signature counts against actual generated artifacts. If any mismatch or missing column is detected, you MUST NOT output the Completeness Signature. Instead, immediately abort and output: `[VALIDATION_FAILED: Mechanical Schema Mismatch]`.
...
If ANY mandatory completeness validation (like `Unaccounted > 0`), referential integrity check, mechanical schema conformance check (Step 6), or implementability quality gate fails and cannot be resolved, DO NOT emit the Completeness Signature. Instead, immediately abort and output: `[VALIDATION_FAILED: <Specific Reason>]`.
