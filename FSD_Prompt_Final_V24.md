...
- If instructed to execute STAGE 1: You are strictly restricted to producing the requested Markdown planning ledger/table. You MUST NOT output any part of the 12-section FSD, contracts, or appendices. Stop immediately after the table.
- If instructed to execute STAGE 2: You will use the previously generated Stage 1 table as your strict baseline to produce the full specification or contract.
...
A behavior must remain a distinct artifact whenever an input, condition, transformation, filter, calculation, branch, ordering, paging, or output can be independently changed. Every distinct observable outcome must remain separately identifiable as a distinct artifact, even when the processing path is otherwise shared. Do not combine independently changeable behaviors into generic statements.

UNIT-TEST ATOMIZATION HEURISTIC:
If a behavior would require an independently meaningful unit, integration, or contract test because its trigger, condition, processing rule, data effect, outcome, or externally observable behavior differs, it MUST be represented as a distinct behavioral artifact. 

SOURCE-BEHAVIOR ACCOUNTING LEDGER (STAGE 1 BASELINE):
Before generation, you must balance this equation:
**Total Inventoried Source Behaviors = Mapped Behaviors + Explicitly Omitted Behaviors + Unaccounted Behaviors.**
- **Item-Level Ledger:** For every inventoried behavior, retain: Behavior ID, behavioral description, source range(s), disposition (Mapped, Explicitly Omitted, or Unaccounted), and target FS artifact ID(s) where mapped. No inventoried behavior may be considered Mapped without a target artifact. *(Note: Do not use "Absorbed" as a source-behavior disposition; "Absorbed" applies exclusively to the Mode 3 BHV Input Disposition Ledger).*
- **Scenario Disposition:** For every distinct outcome in the ledger, explicitly assign a scenario disposition: either target `FS-SCN-<n>` or `NOT-SCENARIO-DISTINCT`.
- Every inventoried behavior must have exactly one disposition.
- **Unaccounted Behaviors MUST equal 0.**
- **CRITICAL SEMANTIC COVERAGE RULE:** "Mapped" does not merely mean an ID was linked. It means the target artifact fully preserves the independently changeable inputs, conditions, transformations, filters, calculations, decisions, branches, ordering, paging, outputs, and side effects of the inventory item.
- Mapping a granular behavior to a high-level Business Intent, Summary (Section 1), or HLR (Section 2) does NOT constitute behavioral coverage.
...
You must objectively define and extract "observable behavior." Include a behavior (and assign it a business rule or flow step) when changing or removing it would change the operation's input acceptance, data selection, filtering, calculations, state mutations, decisions, branching, sorting, pagination, outputs, concurrency, or side effects.
...
**EXACT-DUPLICATE-ONLY MERGING:** Presentation grouping under a Business Intent heading is permitted, but artifact-level merging (combining distinct behaviors into a single `FS-*` ID) is strictly permitted ONLY for exact semantic duplicates. Do NOT compress complex, independently testable behavior into a single high-level artifact. 
...
The main body must answer: What business capability is provided? Why is the process performed? What triggers the process? What information is required? What are the major business activities? What conditions and decisions affect the process? What happens in alternate or failure scenarios? What are the resulting business outcomes? What rules govern the behavior?
...
Implementation-status reporting for fully evidenced artifacts is intentionally out of scope for V24. Appendix C's implementation-status column for such artifacts is deprecated. V24 uses the FSD artifact and its provenance to establish documented functional behavior; Section 11.2 is reserved exclusively for missing or incomplete implementation evidence and explicitly inferred behavior arising from such missing implementations.
...
- **PROVENANCE:** Contains the contributing BHV source-node IDs, Stage 1 Inventory IDs, and/or upstream artifact provenance identifiers. It must preserve the complete provenance chain needed to identify originating behaviors. Do not duplicate auxiliary technical metadata here.
...
**BEHAVIORAL CONTRACT SCHEMA (For Modes 1 & 2 Stage 2):**
...
2. **Observable Behavior:** Triggers, preconditions, conditions/branches, filtering, transformations, ordering, paging, and cardinality. *(A BHV contract may group related behaviors under a single ID, but it MUST NOT collapse independently testable source behaviors merely because they originate from the same method or line range).*
...
- The parent step receives the `FS-MPF-<n>` ID. When the decision table rows provide the complete behavioral evidence for the decision logic, do not duplicate Source/TRACE metadata on the parent step; each decision-table row carries its own Source Tag and inline TRACE.
- Every row in the decision table must be assigned a unique Row ID (e.g., FS-MPF-DEC-<n>), carry a Source Tag, and include its own inline trace tag.
...
*(Qualifying omissions include missing evidence for: null handling, escaping, formatting, mapping, enum/value conversion, response construction, side effects, retries, ordering, concurrency, and other observable helper behavior).*
| Id | Method / Procedure | Source File / Location | Called from / Linked Flow | Call context | Inferred business purpose | Business impact | Action required | Source Tag | Inline TRACE (`<!-- TRACE | ID: FS-MISS-<n> | PROVENANCE: [...] | RANGES: [...] -->`) |
|---|---|---|---|---|---|---|---|---|---|
...
1. **Source-Behavior Accounting:** Verify that mathematically `Total Inventoried = Mapped + Explicitly Omitted + Unaccounted`, and independently verify `Unaccounted = 0`. Verify that all Mapped artifacts semantically preserve independently changeable inputs, conditions, transformations, filters, calculations, decisions, branches, ordering, paging, outputs, and side effects. Confirm every supplied method implementation was fully expanded, leaf-level behavioral integrity is maintained without loss or semantic compression, and flow input/output integrity is preserved.
2. **Universal Provenance Retention:** All child contracts have their exact source coordinate sets and identifiers preserved without truncation across all target artifacts (Invariant 1).
3. **Orchestrator Scope & Semantic Anti-Compression:** Check Invariants 2 and 6. Verify that trace length or token pressure did not cause behavioral atomization to be sacrificed.
...
7. **BA and FE Implementability Tests (Quality Gates):** 
   - Can a BA determine the capability, inputs, decisions, failures, and outcomes? 
   - Can a developer implement the documented behavior without reopening the legacy source? 
   - *If the answer to either is NO, you must internally enrich the functional flow and rules before proceeding.*
