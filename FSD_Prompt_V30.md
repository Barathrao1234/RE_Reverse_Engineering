...
Passive structural translations such as wrapper creation, null-substitution, enum conversions, and passive field population must be primarily routed to `DS-REQ`, `DS-RESP`, or Section 4 flow. Do NOT classify passive mapping as `FS-BRL`. However, if a mapping involves an independently observable decision, condition, default, precedence, validation, or outcome effect, its exact semantics must be preserved without generalization.
