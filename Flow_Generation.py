...
def expand_lineage_horizontal(
    ...
    # class_method_map keys are (full_path_without_ext, methodname)
    ...
    for child in children:
        ...
        for subchild in sub_children:
            if "." in subchild:
                ...
                # impl_cls: append "Impl" to the basename portion only,
                # keeping the full-path prefix intact so method_to_row lookup works
                cls_base    = os.path.splitext(os.path.basename(cls))[0]
                cls_dir     = os.path.dirname(cls)
                impl_cls    = os.path.join(cls_dir, cls_base + "Impl") if cls_dir else cls_base + "Impl"
...
def generate_method_level_hierarchy(
    ...
    log_time(f"Method Flow Generation START")
    ...
    all_paths = []
    java_roots = []
    start_file_names = [item for item in start_file]
    ...
    for ctrl in start_file_names:
        ...
        # ctrl is full path without extension — normalise once
...
#     DROP rows whose Level_1 reappears anywhere else
    ...
    log_time(
        f"Method Flow Generation END | "
        f"Duration={elapsed:.3f} sec"
    ) 
    wb = load_workbook(METHOD_FLOW_OCCURRENCE_DISTRIBUTION_EXCEL)
    ws = wb.active
