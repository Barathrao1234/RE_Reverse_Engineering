...
python reorder_method_flow.py \
    --input  path/to/007_2_Method_Detailed_Flow_Occurrence_Distribution.xlsx \
    --src    path/to/java/source/root \
    --output path/to/007_2_Method_Detailed_Flow_Occurrence_Distribution_REORDERED.xlsx \
    [--ext   .java]          # default .java  (use .py, .cs, etc. as needed)
...
# ─────────────────────────────────────────────
def parse_args():
    p = argparse.ArgumentParser(description="Reorder method flow Excel to match codebase order.")
    p.add_argument("--input",    required=True,  help="Path to input .xlsx")
    p.add_argument("--src",      required=True,  help="Root directory of the source codebase")
    p.add_argument("--output",   required=True,  help="Path for the reordered output .xlsx")
    p.add_argument("--ext",      default=".java",help="Source file extension (default: .java)")
...
def main():
    ...
    rank_map = build_codebase_order(args.src, args.ext, args.encoding)
